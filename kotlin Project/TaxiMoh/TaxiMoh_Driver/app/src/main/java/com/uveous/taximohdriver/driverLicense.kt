...
class driverLicense :AppCompatActivity()
{
    var apitoken: String? = ""
    var userid: Int? = 0
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.driverlicense)
        ...
        if (ContextCompat.checkSelfPermission(this,
            ...
            if (ActivityCompat.shouldShowRequestPermissionRationale(this@driverLicense,
                            Manifest.permission.WRITE_EXTERNAL_STORAGE) && ActivityCompat.shouldShowRequestPermissionRationale(this@driverLicense,
                            Manifest.permission.READ_EXTERNAL_STORAGE)) {
            } else {
                ActivityCompat.requestPermissions(this@driverLicense, arrayOf(Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.READ_EXTERNAL_STORAGE),
                        REQUEST_PERMISSIONS)
            }
        }
    ...
    fun submitfun() {
        ...
        val progressDialog = ProgressDialog(this@driverLicense)
        ...
        val file = File(selectedImagePath)
             ...
             filePart = MultipartBody.Part.createFormData("driving_licence", file.name, requestBody)

        try{
        var mAPIService: ApiService? = null
        mAPIService = ApiClient.apiService
        mAPIService!!.drivinglicense("Bearer "+ apitoken,filePart1, filePart).enqueue(object :
            ...
            override fun onResponse(call: Call<backgroundcheck>, response: Response<backgroundcheck>) {
                ...
                if (response.isSuccessful()) {
                    ...
                    if (lo.status == 200) {
                        ...
                        val i= Intent(this@driverLicense,DriverAcccountSetupSteps::class.java)
                        i.putExtra("token",apitoken)
                        i.putExtra("userid", userid!!)
                        startActivity(i)
                        Toast.makeText(this@driverLicense,"submit", Toast.LENGTH_SHORT).show()
                        Log.v("dd", "post registration to API" + response.body()!!.toString())
                    } else {
                        progressDialog.dismiss()
                        Toast.makeText(this@driverLicense,"error", Toast.LENGTH_SHORT).show()
                    }
            ...
            override fun onFailure(call: Call<backgroundcheck>, t: Throwable) {
                Toast.makeText(this@driverLicense, t.message, Toast.LENGTH_SHORT).show()
            }
        })
