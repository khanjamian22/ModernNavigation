...
class VehicleInsurance :AppCompatActivity()
{
    var apitoken: String? = ""
    var userid: Int? = 0
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.vehicleinsurance)
        ...
        if (ContextCompat.checkSelfPermission(this,
            ...
            if (ActivityCompat.shouldShowRequestPermissionRationale(this@VehicleInsurance,
                            Manifest.permission.WRITE_EXTERNAL_STORAGE) && ActivityCompat.shouldShowRequestPermissionRationale(this@VehicleInsurance,
                            Manifest.permission.READ_EXTERNAL_STORAGE)) {
            } else {
                ActivityCompat.requestPermissions(this@VehicleInsurance, arrayOf(Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.READ_EXTERNAL_STORAGE),
                        REQUEST_PERMISSIONS)
            }
        }
    ...
    fun submitfun() {
        ...
        val progressDialog = ProgressDialog(this@VehicleInsurance)
        ...
        filePart = MultipartBody.Part.createFormData("vehicle_insurance", file.name, requestBody)

        try{
        var mAPIService: ApiService? = null
        mAPIService = ApiClient.apiService
        mAPIService!!.vehicleinsurance("Bearer " + apitoken,filePart1, filePart).enqueue(object :
            ...
            override fun onResponse(call: Call<backgroundcheck>, response: Response<backgroundcheck>) {
                ...
                if (response.isSuccessful()) {
                    ...
                    if (lo.status == 200) {
                        ...
                        val i= Intent(this@VehicleInsurance,DriverAcccountSetupSteps::class.java)
                        i.putExtra("token",apitoken)
                        i.putExtra("userid", userid!!)
                        startActivity(i)
                        Toast.makeText(this@VehicleInsurance,"submit", Toast.LENGTH_SHORT).show()
                        Log.v("dd", "post registration to API" + response.body()!!.toString())
                    } else {
                        progressDialog.dismiss()
                        Toast.makeText(this@VehicleInsurance,"error", Toast.LENGTH_SHORT).show()
                    }
            ...
            override fun onFailure(call: Call<backgroundcheck>, t: Throwable) {
                Toast.makeText(this@VehicleInsurance, t.message, Toast.LENGTH_SHORT).show()
            }
        })
